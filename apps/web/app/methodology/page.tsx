...
const { caseSet, layers, limit } = data as unknown as {
  caseSet: {
    pins: ReadonlyArray<{ pin: string; value: string; same: boolean }>
    runs: ReadonlyArray<{
      label: string
      suite: string
      slug: string
...
const movedPin = caseSet.pins.find((p) => !p.same)
const heldPins = caseSet.pins.filter((p) => p.same)

export default function Methodology() {
  return (
    <Shell>
      <header className="hero">
        <h1 className="hero-title">Why one trigger score is not a measurement</h1>
        <p className="hero-lede">
          Assay prints a trigger accuracy figure, and it is tempting to read that
          figure as <em>the skill works</em>. Two runs stored in this repository
          show the two ways that reading fails. Neither is the fault of a skill:
          in both, the thing that passed was the measurement.
        </p>
      </header>
      ...
      <section className="section-major">
        <p className="rule-label">The first way</p>
        <h2 className="section-title">The case set can be the thing that passes</h2>
        <p className="section-lede">
          The same skill was measured twice on the same day. Three of the four
          pins are byte-identical between the two runs — the skill content hash,
          the model id and the environment. Only the case set changed.
        </p>

        <div className="pin-list mt-10">
          {caseSet.pins.map((pin) => (
            <div key={pin.pin} className={`pin${pin.same ? '' : ' pin-moved'}`}>
              <span className="pin-name">{pin.pin}</span>
              <span className="pin-value">{pin.value}</span>
              <span className="pin-state">{pin.same ? 'held' : 'changed'}</span>
            </div>
          ))}
        </div>
        <p className="section-note">
          {heldPins.length} of {caseSet.pins.length} pins held.{' '}
          {movedPin === undefined
            ? 'Every pin held.'
            : `Only ${movedPin.pin} moved — the hash of the case set file.`}
        </p>
        ...
        <p className="section-note">
          Precision is the share of firings that should have happened. The first
          set reports it as{' '}
          <strong>
            {firstSet?.precision.successes}/{firstSet?.precision.n}
          </strong>
          ; the second, measuring the same skill under the same conditions,
          reports{' '}
          <strong>
            {secondSet?.precision.successes}/{secondSet?.precision.n}
          </strong>
          . Recall is unchanged in both. The whole difference is in how the
          negative cases were cut.
        </p>

        <p className="section-note">
          Reading the traces explains it. In the first set every negative case
          put a document in front of the agent to transform — code, prose, a list
          of commits — while every positive case described work that existed only
          in the user&apos;s head. The set was separating cases on that second
          axis, not on the property it meant to test, and the skill happened to
          be flawless on it. Re-cutting the negatives so they differ from the
          positives on the tested property <em>and nothing else</em> moved the
          number by half.
        </p>

        <Callout tone="info" title="What this changes about reading a result">
          A trigger suite that passes every case is a statement about the suite
          as much as about the skill. Assay enforces that a case set contains a
          negative and a near neighbour; it cannot yet check that the near
          neighbour is near along the right axis. Until it can, a clean sheet is
          a prompt to look at the negatives, not a reason to stop.
        </Callout>
      </section>
      ...
      <section className="section-major">
        <p className="rule-label">The second way</p>
        <h2 className="section-title">The layer can be the thing that passes</h2>
        <p className="section-lede">
          Trigger accuracy answers one question: did it fire? A second run asked
          the next one — once it fired, did the work get finished? Across{' '}
        ...
        <p className="section-note">
          No attempt landed in either of the other two cells. Where the skill
          engaged, the file was never written; where it stayed out of the way,
          the file was always written.
        </p>
        ...
        <div className="mt-12">
          <p className="col-label mb-4">
            The whole trace of one such attempt — {layers.trace.steps.length} events,
            no <code className="code">Write</code> among them
          </p>
        ...
        <Callout tone="info" title="This is the skill behaving as documented">
          Its own description is a multi-turn workflow whose first stage is
          gathering context by asking the user. A single-turn CI run has nobody to
          answer, so it ends where the workflow says it should end. Nothing here
          is broken. What is wrong is a measurement that stops at the first layer
          and reports the result as completion.
        </Callout>
      </section>
      ...
      <section className="section-minor">
        <p className="rule-label mb-6">Isolating the variable</p>
        <h2 className="section-title">The control case</h2>
        <p className="section-lede">
          Both readings above are causal claims, and a causal claim needs the
          other variables held still. The completion run carries a control case
          for exactly that: its prompt is word-for-word identical to{' '}
          <code className="code">{designDoc.caseId}</code> except for the final
          paragraph, which asks for a file on disk. Nothing else differs — same
          run, same record, same four pins.
        </p>
        <div className="ruled mt-8">
          <MetricValue
            label="control · no file requested · fired"
            value={control.trigger}
            tone="text-pass"
          />
          <MetricValue
            label="same prompt + &quot;save it to out/…&quot; · fired"
            value={designDoc.trigger}
          />
        </div>
        <p className="section-note">
          Because the two cases sit in one run, the comparison does not ask you to
          trust the person reporting it: the pins are recorded once and cover
          both. Running the control separately would have produced a second
          record with its own environment hash, and the claim would have rested
          on an assurance instead of a pin.
        </p>
      </section>
      ...
      <section className="section-minor">
        <p className="rule-label mb-6">The limit of this instrument</p>
        <h2 className="section-title">A file existing is not proof its content was earned</h2>
        <p className="section-lede">
          The completion layer asserts that a file exists, parses, and matches a
          pattern. It cannot assert where the content came from. One case in a
          third run makes the gap visible.
        </p>
        <dl className="limit-grid mt-8">
          ...
          <div className="limit">
            ...
            <dd className="limit-body">
              attempts had at least one shell call refused by the host&apos;s
              permission layer, so the run mostly could not happen
            </dd>
          </div>
          <div className="limit">
            <dt className="limit-figure">
        ...
        <p className="section-note">
          Both figures are honest and they disagree. That disagreement is the
          finding: <code className="code">file_exists</code> plus{' '}
          <code className="code">file_content_matches</code> can tell you a
          plausible file is there; they cannot tell a recorded run from a
          well-formed guess. The same boundary is why side-effect claims return{' '}
          <span className="text-unknown">unknown</span> whenever a run used a
          shell — an unobserved write is not a clean one.
        </p>
        <Callout tone="warning" title="Stated, not fixed">
          ...
          instrument that hides its own blind spot is worse than one that has a
          bigger one.
        </Callout>
      </section>
      ...
      <section className="section-minor section-last">
        ...
        <p className="section-lede">
          Every figure on this page is read out of a stored run record. None is
          written by hand.
        </p>
        ...
        <p className="section-note">
          Regenerate with{' '}
          <code className="code">
            node tools/methodology-data.mjs &lt;trigger-root&gt;
            &lt;completion-root&gt; apps/web/app/methodology/measurements.json
          </code>
          . The case sets live under <code className="code">suites/</code> and{' '}
          <code className="code">examples/measurements/</code>; the long-form
          reports are in <code className="code">docs/measurements.md</code>.
        </p>
        <p className="mt-8">
