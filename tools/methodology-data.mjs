...
import { containsHomePath, redactDeep } from '../packages/core/dist/redact.js'
...
function load(root, name) {
  ...
  /*
   * Maskeleme burada da uygulanıyor. Kayıtların bir kısmı ev dizini
   * maskelemesi eklenmeden ÖNCE yazıldı ve iz metinleri mutlak yollarla dolu.
   * Sayfa herkese açık: o kayıtları olduğu gibi basmak işletim sistemi
   * kullanıcı adını yayımlamak olurdu.
   */
  const run = redactDeep(JSON.parse(readFileSync(join(dir, newest), 'utf8')).run)
  return { slug: newest.replace(/\.json$/, ''), run }
}

/** Nesnedeki her metni tek tek denetler: JSON'a çevrilmiş hâli değil. */
function findLeak(value, path = '$') {
  if (typeof value === 'string') return containsHomePath(value) ? path : null
  if (Array.isArray(value)) {
    for (const [i, item] of value.entries()) {
      const hit = findLeak(item, `${path}[${i}]`)
  ...
  if (typeof value === 'object' && value !== null) {
    for (const [key, item] of Object.entries(value)) {
      const hit = findLeak(item, `${path}.${key}`)
      if (hit !== null) return hit
    }
  }
  return null
 ...
 * Dört pin, altı alan.
 *
 * Değişmez #2 dört pin sayıyor; kayıt altı alan taşıyor çünkü iki pinin bir
 * de denetçisi var (beyan edilen sürüm unutulur, içerik hash'i unutmaz).
 * Sayfa bunu dört pin olarak göstermeli, yoksa "dört pinden üçü" ile
 * "altıdan beşi" aynı paragrafta çelişiyor.
 *
 * Üç durum var, iki değil. Host'un vermediği bir alan iki koşumda da aynı yer
 * tutucuyu taşıyor; onu "tuttu" saymak, ölçülmemiş bir koşulu ölçülmüş gibi
 * göstermek olurdu — sayfanın kendi argümanının ihlali.
 */
const UNAVAILABLE = new Set(['', 'not-provided-by-host', 'unknown', 'n/a'])

const PINS = [
  { pin: 1, name: 'Skill version', fields: ['skillSource', 'skillHash'] },
  { pin: 2, name: 'Model identity', fields: ['model'] },
  { pin: 3, name: 'Host environment', fields: ['systemPromptHash', 'environmentHash'] },
  { pin: 4, name: 'Case set version', fields: ['suiteVersion', 'suiteHash'] },
]

const fieldState = (key) => {
  const a = baseline.run.pins[key]
  const b = borderline.run.pins[key]
  ...
  return {
    field: key,
    value: String(a ?? ''),
    state: blind ? 'unavailable' : a === b ? 'held' : 'changed',
  }
}

const pinRows = PINS.map(({ pin, name, fields }) => {
  const states = fields.map(fieldState).filter((f) => f !== null)
  const state = states.some((f) => f.state === 'changed')
    ? 'changed'
    : states.every((f) => f.state === 'unavailable')
      ? 'unavailable'
      : 'held'
  return { pin, name, state, fields: states }
})
...
const caseSet = {
  ...
  counts: {
    ...
    held: pinRows.filter((p) => p.state === 'held').length,
    changed: pinRows.filter((p) => p.state === 'changed').length,
    unavailable: pinRows.filter((p) => p.state === 'unavailable').length,
  },
...
// Son bir denetim: üretilen dosyada maskelenmemiş bir ev dizini kalmasın.
const payload = { caseSet, layers, limit }
const leak = findLeak(payload)
if (leak !== null) {
  process.stderr.write(`leak: an unredacted home path survived at ${leak}\n`)
  process.exit(1)
}
